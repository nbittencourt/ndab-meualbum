...
Tela dedicada à gestão dos álbuns do usuário. Acessada a partir da Home via ação de navegação explícita. Oferece lista completa dos álbuns ativos, gerenciamento individual de cada álbum (popup de figurinhas que faltam com opção de impressão A4, arquivamento) e seção de álbuns arquivados.
...
[Tela AL0 — Lista de Álbuns]
  ...
  Lista paginada de álbuns ativos + seção de arquivados (condicional)
        ...
        ├── [Card ativo — "Figurinhas que faltam"] → abre popup de figurinhas que faltam (com botão Imprimir) (ver RN-AL30)
...
[Tela AL1 — Gerenciamento do Álbum]
        ...
        ├── ["Figurinhas que faltam"] → abre popup de figurinhas que faltam (com botão Imprimir)
...
- **"Figurinhas que faltam"** — abre popup modal de figurinhas que faltam (ver 5.4)
...
### 5.4 Popup de Figurinhas que Faltam (e impressão)

Aberto ao acionar "Figurinhas que faltam" — disponível na AL0 (card) e na AL1 (barra de ações). Não há download automático de PDF: o popup é exibido imediatamente, refletindo o estado atual do álbum (sem cache).

**Conteúdo do popup:**
- Grade completa de todas as figurinhas do álbum, organizadas por seção em ordem `Secao.ordem ASC`
- Cada figurinha exibe: número e estado visual (`colada` / `faltante` / `repetida`)
- Totais: coladas, faltantes, repetidas e percentual de conclusão
- Botão **Imprimir** no cabeçalho do popup — `onClick: window.print()`; oculto na própria impressão

**Layout de impressão A4 (retrato):**
- Gerado via `window.print()` (PDF via "Salvar como PDF" do navegador)
- Cabeçalho: marca "Meu Album", nome do álbum, data de geração
- Faixa de stats: coladas / faltantes / repetidas / percentual
...
| RN-AL07 | O popup e a impressão listam todas as figurinhas do álbum (coladas, faltantes e repetidas) refletindo o estado atual no momento do acesso |
| RN-AL08 | Seções completamente preenchidas continuam exibidas no popup (com destaque visual de conclusão) |
...
| RN-AL17 | O popup reflete o estado atual do álbum no momento da abertura; não há cache da listagem de figurinhas entre aberturas |
...
| RN-AL19 | Ao acionar "Figurinhas que faltam", o popup modal é aberto imediatamente — sem spinner bloqueante, sem download automático. O popup exibe a grade completa (coladas/faltantes/repetidas) com totais e botão Imprimir (A4 via `window.print`). O `dialog` DEVE ter `role="dialog"`, `aria-modal="true"`, `aria-label` identificando o álbum e `lang="pt-BR"` |
...
| RN-AL30 | O botão "Figurinhas que faltam" está disponível tanto na Tela AL0 (card de álbum ativo) quanto na Tela AL1 (barra de ações). Em ambos os contextos, o comportamento é idêntico: abre popup modal com a grade completa do álbum e botão Imprimir. Não há navegação para outra tela nem download automático |
| RN-AL31 | ~~DEPRECIADO (issue #29)~~ — O botão "Ver Álbum" e a Tela AL2 foram removidos. A funcionalidade de visualização completa por seção está integrada ao popup de figurinhas (RN-AL30) |
...
| Carregamento do popup de figurinhas | Spinner dentro do popup; seções exibidas ao carregar |
...
| Falha ao carregar figurinhas no popup | Mensagem de erro inline dentro do popup; botão Imprimir omitido enquanto sem dados |
...
| RN-AL22 | O popup de figurinhas DEVE ser fechável via botão "Fechar" interno, clique no backdrop e tecla Escape; ao fechar, o foco DEVE retornar ao botão que abriu o popup |
...
| RN-AL27 | O container de impressão DEVE ter `aria-hidden="true"` (é uma cópia visual para impressão, não deve ser lida por leitores de tela) e o `dialog` na tela já provê acessibilidade completa ao conteúdo |
