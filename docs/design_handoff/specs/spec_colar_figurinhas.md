...
|---|---|---|
| `origem` | Enum | `ESTOQUE` — figurinha debitada do estoque do usuário; `DIRETA` — figurinha informada diretamente no fluxo, sem passar pelo estoque |

> **Impacto nas specs anteriores:** a entidade `FigurinhaColada` referenciada em Abrir Pacotinhos e Home recebe este campo adicional. Registros criados pelo fluxo Abrir Pacotinhos recebem `origem = ESTOQUE` (sempre provêm do estoque após Abrir Pacotinhos descarregar ali). Registros criados pelo presente fluxo recebem `origem = ESTOQUE` ou `DIRETA` conforme o caminho percorrido.
...
Cada item do estoque exibe um dos três estados:

| Estado | Condição | Visual sugerido |
|---|---|---|
| **Pode colar** | Figurinha não possui registro em `FigurinhaColada` para o álbum ativo | Indicador positivo (ex.: badge verde) |
| **Já colada** | Figurinha já possui registro em `FigurinhaColada` para o álbum ativo | Indicador de alerta (ex.: badge amarelo ou ícone de aviso) |
| **Fora do catálogo** | `figurinha.tipo_album_id` ≠ `tipo_album_id` do álbum ativo | Item desabilitado; ação de colar não disponível |
...
| RN-CF01 | Apenas usuários com `status = ATIVO` acessam o fluxo; `PENDENTE` é redirecionado para a Tela 2 de Confirmação de Email |
